### google maps is incredible

#### i used to play geoguessr hours on end every day during the lockdown. i developed a fascination with the world around us and still wander google maps occasionally in my free time. i will add interesting locations & pictures i stumble across here, from street view to aerial views to regular flicks.

<h4> (interactive) </h4>

<br/> 

<!-- 1 -->
<div style="display: flex; width: 100vw; flex-direction: column;">
    <div style="display: flex; align-items: center; gap: 10px; padding-bottom: 10px;">
        <h3 style="margin: 0; line-height: 1;">1. salar de uyuni - bolivia</h3>
        <span style="font-size: 40px; line-height: 1;">🇧🇴</span>
    </div>

<iframe src="https://www.google.com/maps/embed?pb=!4v1742368165285!6m8!1m7!1sCAoSLEFGMVFpcE56MmFfQjltUnc5SFZ1R0hGeFVOdnFkeG0xQTRjYTdsaXFqbDR0!2m2!1d-20.23997223102578!2d-67.58111002249453!3f338.04650672981865!4f-19.583627866554025!5f0.43744515973550396" 
            style="width: 90vw; height: 75vh; border: 0;" 
            allowfullscreen="" loading="lazy" referrerpolicy="no-referrer-when-downgrade"></iframe>

<h4 style="margin-top: 10px; margin-bottom: 25px;">biggest salt flat in the world that can be seen from space. Always found it eye-catching,  mesmerizing.</h4>
<hr style="margin: 0; padding:0; margin-bottom: 12px;"/>
</div>

<!-- 2 -->
<div style="display: flex; width: 100vw; flex-direction: column;">
    <div style="display: flex; align-items: center; gap: 10px; padding-bottom: 10px;">
        <h3 style="margin: 0; line-height: 1;">2. parque natural san carlos de apoquindo - chile</h3>
        <span style="font-size: 40px; line-height: 1;">🇨🇱</span>
    </div>

<iframe src="https://www.google.com/maps/embed?pb=!4v1742369672161!6m8!1m7!1sn9ZN2hJa771b4NBDz1PY5w!2m2!1d-33.39651916405647!2d-70.45709398232871!3f330.05!4f-7.609999999999999!5f0.7820865974627469" 
            style="width: 90vw; height: 75vh; border: 0;" 
            allowfullscreen="" loading="lazy" referrerpolicy="no-referrer-when-downgrade"></iframe>

<h4 style="margin-top: 10px; margin-bottom: 25px;">mountain range right next to the borders of santiago, chile's capital.</h4>
<hr style="margin: 0; padding:0; margin-bottom: 12px;"/>
</div>

<!-- 3 -->
<div style="display: flex; width: 100vw; flex-direction: column;">
    <div style="display: flex; align-items: center; gap: 10px; padding-bottom: 10px;">
        <h3 style="margin: 0; line-height: 1;">3. mountainous district - portugal</h3>
        <span style="font-size: 40px; line-height: 1;">🇵🇹</span>
    </div>

<iframe src="https://www.google.com/maps/embed?pb=!4v1742416120061!6m8!1m7!1sHJJ20QRfuTeYGRRTn_lukw!2m2!1d41.27517404103741!2d-7.859529744857597!3f183.22189633540813!4f5.8944974875990965!5f0.8154479948595579" 
            style="width: 90vw; height: 75vh; border: 0;" 
            allowfullscreen="" loading="lazy" referrerpolicy="no-referrer-when-downgrade"></iframe>

<h4 style="margin-top: 10px; margin-bottom: 25px;">houses deeply nestled in the mountainside in rural portugal. </h4>
<hr style="margin: 0; padding:0; margin-bottom: 12px;"/>
</div>

<!-- 4 -->
<div style="display: flex; width: 100vw; flex-direction: column;">
    <div style="display: flex; align-items: center; gap: 10px; padding-bottom: 10px;">
        <h3 style="margin: 0; line-height: 1;">4. kazamaura, aomori - japan</h3>
        <span style="font-size: 40px; line-height: 1;">🇯🇵</span>
    </div>

<iframe src="https://www.google.com/maps/embed?pb=!4v1742522944358!6m8!1m7!1sW75jxFiQTqRxGhaS4j29nA!2m2!1d41.47529215239408!2d141.077070760996!3f77.55!4f-2.230000000000004!5f0.7820865974627469" 
            style="width: 90vw; height: 75vh; border: 0;" 
            allowfullscreen="" loading="lazy" referrerpolicy="no-referrer-when-downgrade"></iframe>

<h4 style="margin-top: 10px; margin-bottom: 25px;">very northern tip of aomori japan, the coverage here feels reminiscent/nostalgic, of something... </h4>
<hr style="margin: 0; padding:0; margin-bottom: 12px;"/>
</div>

# ...
<br/>